...
def train(
        model, train_dataloader, valid_dataloader, total_epochs,
...
if __name__ == '__main__':
    ...
    audio_model = BiLSTM(input_size=audio_input_dim, hidden_1=HIDDEN_1, hidden_2=HIDDEN_2, linear=True, out_size=32)
    image_model = swinv2_tiny_window16_256(pretrained=True)
    image_dim = image_model.head.in_features
    image_model.head = nn.Linear(image_dim, 32)

    encoders = [audio_model.to(device), image_model.to(device)]
    head = MLP(em_dim, mlp_em_dim, class_num).to(device)
    fusion = LowRankTensorFusion([32, 32], em_dim, rank).to(device)
    model = MMDL(32, encoders, fusion, head).to(device)
